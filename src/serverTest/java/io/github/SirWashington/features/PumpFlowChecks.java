...
final class PumpFlowChecks implements AutoCloseable {
    ...
    private void basic(Direction facing, int size) {
        ...
        boolean pathMode = WaterPhysicsConfig.pumpStraightOnly();
        try {
            ...
            WaterPhysicsConfig.SERVER.pump.straightOnly.set(pathMode);
        }
    }
    ...
    private void partialPrimingMustDischarge() {
        ...
        try {
            for (boolean straight : new boolean[]{false, true}) for (Direction facing : Direction.values()) {
                WaterPhysicsConfig.SERVER.pump.straightOnly.set(straight);
                clear();
                ...
                int before = total();
                PumpFlow.tick(level, ORIGIN);
                expect(totalBeyond(facing, 3) == 7, "One missing internal level must not cancel discharge: " + facing + " straight " + straight);
        ...
        try {
            for (boolean straight : new boolean[]{false, true}) for (Direction facing : Direction.values()) {
                ...
                for (int size = 1; size <= 3; size++) {
                    ...
                    expect(totalBeyond(facing, 3) > 0, "Series must discharge with natural flow: " + facing + " size " + size + " straight " + straight);
                }
            }
        } finally { WaterPhysicsConfig.SERVER.pump.straightOnly.set(previous); }
        System.out.println("PUMP_SERIES_NATURAL_FLOW_ALL_MODES_PASS");
    }

    private void downstreamAndStraight() {
        boolean previous = WaterPhysicsConfig.pumpStraightOnly();
        try {
            for (boolean straight : new boolean[]{false, true}) {
                WaterPhysicsConfig.SERVER.pump.straightOnly.set(straight);
                ...
                expect(!PumpFlow.transfer(level, stages, 16, 64), "Independent powered downstream pump blocks pressure in mode " + straight);
                ...
                expect(PumpFlow.transfer(level, stages, 8, 64) == !straight, "Only BFS can use a bent outlet");
                if (straight) expect(FiniteWaterPhysics.getWaterLevel(level, ORIGIN.below()) == 8, "Failed straight scan preserves intake");
            }
        } finally { WaterPhysicsConfig.SERVER.pump.straightOnly.set(previous); }
        System.out.println("PUMP_DOWNSTREAM_BOUNDARY_AND_STRAIGHT_MODE_PASS");
    }
    ...
    boolean tick() {
        ...
        if (level.getGameTime() % 20 == 0) cycles++;
        expect(totalBeyond(Direction.UP, 3) == Math.max(0, cycles - 3) * 8,
                "Server cadence: prime three stages, then discharge 8 units per 20 ticks");
